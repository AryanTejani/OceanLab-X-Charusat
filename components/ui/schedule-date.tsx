...
export const ScheduleDate: React.FC<ScheduleDateProps> = ({ onApply, onCancel }) => {
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <div key={day} onClick={() => handleDateClick(currentDayDate)} className="relative h-8 flex items-center justify-center group cursor-pointer">
                                ...
                                {(isStart || isEnd) ? (
                                    <div className="absolute w-8 h-8 rounded-lg bg-linear-to-b from-neutral-700 to-neutral-900 dark:from-neutral-800 dark:to-indigo-900/50 border border-slate-200 border-neutral-600 dark:border-white/10 shadow-xl z-10 flex flex-col items-center justify-center dark:border-slate-800">
    ...
    return (
        <div className="flex flex-col w-full max-w-195 bg-white dark:bg-black border border-slate-200 border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden font-sans text-neutral-600 dark:text-neutral-400 shadow-2xl mx-auto  dark:border-slate-800">
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    selectedPreset === preset.id
                                        ? (preset.id === 'custom'
                                            ? 'bg-linear-to-b from-neutral-100 to-neutral-200 dark:from-neutral-800 dark:to-neutral-900 border border-slate-200 border-neutral-300 dark:border-neutral-700 text-neutral-900 dark:text-white font-medium dark:border-slate-800'
            ...
            <footer className="h-16 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-950/50 flex items-center justify-end px-6 gap-3 shrink-0">
                <button onClick={onCancel} className="px-4 py-1.5 rounded-full border border-slate-200 border-neutral-200 dark:border-neutral-800 text-xs font-medium text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900 hover:text-neutral-900 dark:hover:text-white transition-colors dark:border-slate-800">Cancel</button>
...
const DateInput = ({ label, date }: { label: string, date: Date | null }) => (
    <div className="flex-1 flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-between bg-neutral-50 dark:bg-neutral-900 border border-slate-200 border-neutral-200 dark:border-neutral-800 rounded-xl px-3 py-2 text-xs md:text-[13px] text-neutral-600 dark:text-neutral-300 cursor-pointer hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors dark:border-slate-800">
